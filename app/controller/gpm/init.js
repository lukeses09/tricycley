var validator = $('#form_gpm').validate();
...
function load(){
page_name("GPM Record Management");
populate_table_main();
menu_name2("manage","manage_gpm");
}
...
function populate_table_main(){ 
	...
	$.ajax ({
	  type: "POST",
	  url: "../../model/gpm/populate_table_main.php",
	  dataType: 'json',      
	  cache: false,
	  success: function(s)	
	  {
	    ...
	    { 
	      ...
	      ([
	        comma(s[i][1]),comma(s[i][2]),comma(s[i][3])+' %',s[i][4],btn_update+btn_remove,
	      ],false); 
	      table_main.fnDraw();
...
function save_form_gpm(){
	if($('#form_gpm').valid()==true){ // will save
		disable_save_buttons_prop(true);
		var f_gross = $('#f_gross').val();
		var f_net = $('#f_net').val();
		// var f_gpm = $('#f_gpm').val();
		var f_date = $('#f_year').val()+"-"+$('#f_month').val()+"-01";

		var dataString = 'f_gross='+f_gross+'&f_net='+f_net;
		dataString+='&f_date='+f_date;
	...
	if(mode=='CREATE'){
		...
		$.ajax ({
		  type: "POST",
		  url: "../../model/gpm/create.php",
		  data: dataString,
		  // dataType: 'json',      	  
		  success: function(x){ 
		  	if(x==0)
				swal('Saved!', 'New GPM Record Created!','success'); 
			else if(x==1)
				swal('Failed!', 'Something went wrong...','error'); 	
			else if(x==2)
				swal('Duplicate!', 'Cannot be saved, GPM Record with the same MONTH & YEAR already exist in the datatables','warning'); 
		...
		$.ajax ({
		  type: "POST",
		  url: "../../model/gpm/update.php",
		  data: dataString+'&id='+mode,
		  // dataType: 'json',      
		  success: function(x){ 
		  	if(x==0)
				swal('Updated!', 'GPM Record has been Updated!','success'); 
			else if(x==1)
				swal('Failed!', 'Something went wrong...','error'); 			
			populate_table_main(); //refresh table
			clear_form_gpm();											
...
function delete_form_gpm(id){
	//ajax now
	$.ajax ({
	  type: "POST",
	  url: "../../model/gpm/delete.php",
	  data: 'id='+id,
	  // dataType: 'json',      
	  cache: false,
	  success: function(x){ 
	  if(x==0){swal("Deleted!", "GPM record has been successfully deleted", "success");} 
	  if(x==1){swal("Failed!!", "Something went wrong...", "error");} 
	  if(x==2){swal("Cannot Be Deleted!", "GPM is recorded to another Document", "warning");} 		  	
  	  populate_table_main();
	  },
