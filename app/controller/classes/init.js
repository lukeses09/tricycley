...
function load(){
menu_settings.page_name("Class");
populate_table_main();
menu_settings.menu_name2("maintenance","class");
dropdown_instructor()
}
...
function populate_table_main(){
  ...
  function dropdown_instructor(){
   //ajax now
   $.ajax ({
     type: "POST",
     url: "../../model/class/dropdown_instructor.php",
     dataType: 'json',
     cache: false,
     success: function(s)
     {
       ...
       for(var i = 0; i < s.length; i++) {
       	// s[i][0] ay id
       	// s[i][1] ay name
        $('#f_instructor').append('<option id="opt'+s[i][0]+'" value="'+s[i][0]+'">'+s[i][1]+'</option>');
      }
    }
  });
  //ajax end
...
function save_form_class(){
    ...
    var dataString =  {
      ...
      // f_duration : $(`#f_duration`).val(),
      f_charge : $(`#f_charge`).val(),
      f_start_time : $(`#f_start_time`).val(),
      f_end_time : $(`#f_end_time`).val(),
      f_instructor : $(`#f_instructor`).val()
    }
...
function row_update(id){
	...
	$.ajax ({
	  type: "POST",
	  url: "../../model/class/fetch.php",
	  data: 'id='+id,
	  dataType: 'json',
	  cache: false,
	  success: function(s){
	  	$('#f_name').val(s[0][1]);
	  	// $('#f_description').val(s[0][2]);
	  	// $('#f_duration').val(s[0][3]);
	  	$('#f_charge').val(s[0][2]);
	  	$('#f_start_time').val(s[0][3]);
	  	$('#f_end_time').val(s[0][3]);
	  	$('#f_instructor').val(s[0][5]);
	  	$('#btn_save').text('UPDATE');
	  }
	});
	// //ajax end
