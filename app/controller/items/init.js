...
function load(){
menu_settings.page_name("Products");
populate_table_main();
menu_settings.menu_name2("maintenance","items");
}
...
function populate_table_main(){
	...
	$.ajax ({
	  type: "POST",
	  url: "../../model/item/populate_table_main.php",
	  dataType: 'json',
	  cache: false,
	  success: function(s)
	  {
	    ...
	    for(var i = 0; i < s.length; i++)
	    {
	      ...
	      ([
	        s[i][1],s[i][2],s[i][3],btn_update+btn_remove,
	      ],false);
	      table_main.fnDraw();
...
function datalist_category(){
  ...
  $.ajax ({
    type: "POST",
    url: "../../model/item/datalist_category.php",
    dataType: 'json',
    cache: false,
    success: function(s)
    {
