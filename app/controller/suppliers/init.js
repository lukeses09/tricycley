...
function load(){
menu_settings.page_name("Suppliers");
populate_table_main();
menu_settings.menu_name2("maintenance","suppliers");
}
...
function populate_table_main(){
	...
	$.ajax ({
	  type: "POST",
	  url: "../../model/supplier/populate_table_main.php",
	  dataType: 'json',
	  cache: false,
	  success: function(s)
	  {
...
function save_form_class(){
    ...
    var dataString =  {
      ...
      f_contact_person : $(`#f_contact_person`).val(),
      f_contact_no: $(`#f_contact_no`).val()
...
function row_update(id){
	...
	$.ajax ({
	  ...
	  success: function(s){
	  	$('#f_name').val(s[0][1]);
	  	$('#f_address').val(s[0][2]);
		$('#f_contact_person').val(s[0][3]);
	 	$('#f_contact_no').val(s[0][4]);
	  	$('#btn_save').text('UPDATE');
	  }
	});
	// //ajax end
